    ...
    <div class="card-body wizard-content">
        ...
        <form action="{{ route('backend.'.request()->segment(3).'.update',$row->id) }}" method="post"
            ...
            <form action="{{ route('backend.'.request()->segment(3).'.store') }}" method="post"
                ...
                <div class="row">
                    <label for="complaint_text">{{ __('table.message') }} </label>
                    <textarea  @isset($row)
                    readonly
                    @endisset cols="80" id="editor1" name="complaint_text" rows="3" data-sample="2" class="form-control" data-sample-short>{{ strip_tags($row->message,"<p>") ?? old('message') }}</textarea>
